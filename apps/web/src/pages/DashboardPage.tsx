...
export function DashboardPage() {
  ...
  const isLocalFallback = aiInsightsQuery.data?.model === 'local-heuristic';
          ...
          <div className="mt-3 space-y-3">
            {isLocalFallback ? (
              <p className="rounded-lg bg-poke-yellow/20 px-3 py-2 text-sm text-poke-dark/80">
                AI providers are unavailable{quotaWarning ? ' (quota exceeded)' : ''}. Showing offline
                insights based on your collection instead.
              </p>
            ) : (
              quotaWarning && (
                <p className="rounded-lg bg-poke-yellow/20 px-3 py-2 text-sm text-poke-dark/80">
                  {providerLabel(quotaWarning.provider)} quota exceeded. Showing fallback provider results
                  instead.
                </p>
              )
            )}
            ...
            {isLocalFallback && (
              <p className="text-xs text-poke-dark/45">Generated offline · local-heuristic</p>
            )}
          </div>
        )}
      </Card>
